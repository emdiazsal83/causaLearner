...
experimentName <- "dag2-ME2-Cmplx_cmem-none-CV"
...
proc.time() - pm # 2 hours

# 15 mins for none_1      with 1-1-6 cores on my pc with 1-1 in CV
# for kernParsX_1 with 1-1-6 cores on my pc with 1-1 in CV

kernel <- strsplit(recipe$hypScorer[1],"_")[[1]][4:5]
kernel <- paste(strsplit(kernel[1],"AND")[[1]][1], kernel[2], sep="_")
save(scores, file=paste("./pkg_causaLearner/experiments/CMEM/results/", experimentName, "_", hs_cmem_ob_version,"_",kernel, ".RData", sep=""))
...
statsDB$sigmaX <- rbfX$sigma[as.numeric(sapply(strsplit(statsDB$hypSc, "_"), function(el) el[10]))]
statsDB$scaleY <- laplace2Y$scale[as.numeric(sapply(strsplit(statsDB$hypSc, "_"), function(el) el[11]))]


msrss <- c("ccr")
stats <- c("orig")
indx <- with(statsDB, which(measure %in% msrss & statistic %in% stats))
cast(statsDB[indx,], hypSc+sigmaX+scaleY+lambda~cmplxFunc, fun.aggregate="length")
tab <- cast(statsDB[indx,], sigmaX+scaleY+lambda~cmplxFunc, value="value")
tab

tab[which(tab$sigmaX==0.0001),]
tab[which(tab$sigmaX==1),]
tab[which(tab$sigmaX==10000),]

lambdaIndx <- as.numeric(sapply(strsplit(tab$hypSc, "_"), function(el) el[9]))
tab <- tab[order(lambdaIndx),]
tab
plot(log(lambdas, 10), tab$KCDC,  type="l")
lines(log(lambdas, 10), tab$KCMC, col="red")
...
p <- p + geom_line(aes(x=log(lambda,10), y=value, color=cmplxFunc))
