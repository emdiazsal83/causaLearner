...
experimentName <- "dag2-ME2-TCEP_cmem-comp-4-l2"
...
# this was a way to correct results since createTCEP2
# was wrong - DONT RUN ANYMORE SINCE IVE FIXED IT
# lapply(dataList$xs, colnames)
# # correct dags
# dagXY <- matrix(c(0,0,1,0),2,2)
# dagYX <- matrix(c(0,1,0,0),2,2)
# dags <- lapply(dataList$xs, function(x){
#   if(all(colnames(x)==c("x","y"))) dag <- dagXY else dag <- dagYX
...
#version <- "vNC_CLASS"
#version <- "vNC_REG"
version <- "v_nn_12_10" #"v_none" #"vNC_CORSPEARMAN"
