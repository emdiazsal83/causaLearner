...
experimentName <- "dag5-ME32-Cmplx_cmemJ-none"
...
hypScorerList <- c(cmemJ_hypScorer_pack_none_1, "rnd1") #[c(5,6)]
...
eval(parse(text=cmemJ_hypScorer_pack_none_1[1]))
...
indx <- grep( paste(experimentName, hs_cmem_ob_version, sep="_"), blockFiles)
...
save(scores, file=paste("./pkg_causaLearner/experiments/CMEM/results/", experimentName,"_",hs_cmem_ob_version,"_lambda_1e",lambda,".RData", sep=""))
...
#########################################################################################################################################*
lambda <- log(eval(parse(text=eval(parse(text=recipe$hypScorer[1]))$cmemLearner))$hyperParams$data$non_optimizable$lambda$val,10)
load(file=paste("./pkg_causaLearner/experiments/CMEM/results/", experimentName,"_",hs_cmem_ob_version,"_lambda_1e",lambda, ".RData", sep=""))
...
scores[[1]][["norm"]][["oracleME1"]][[1]][,c("KCDC","KCRDC","KCMC","KCSC","KCNSC")]
  ...
  scrs <- lapply(1:length(scores[[i]]), function(j){
    ...
    scrs <- lapply(1:length(scores[[i]][[j]]), function(k){
      ...
      scrs <- lapply(1:(length(scores[[i]][[j]][[k]])-1), function(l){
        # l <- 1
        scores[[i]][[j]][[k]][[l]][,c("KCDC","KCRDC","KCMC","KCSC","KCNSC")]
      })
