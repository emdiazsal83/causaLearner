...
experimentName <- "dag5-ME32-Mult_cmem-none"
...
# plot(as(dagMatME[,,2], "graphNEL"))
 ...
 # set.seed(5)
 # dataList <- simRandSEMs(q, ps, ns, nodess, sigma=1, dagMat=dagMat, markovEquiv=T, geU=function(y, nois, scale, constant) y*nois)
 # pairs(dataList$xs[[3]])
...
sapply(cmem_hypScorer_pack_none_1, function(el) eval(parse(text=el))$func)
...
pm <- proc.time() 
scores <- applyLearnRecipesOne(recipe, data=dat, numCoresDt=1, numCoresHypSc=2, plot=TRUE)
proc.time()-pm 
