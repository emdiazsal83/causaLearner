...
experimentName <- "dag2-ME2-ANLSMN_qkr"
...
#folderSave <- "/home/emiliano/causaLearner/pkg_causaLearner/experiments/blocks/"
folderSave <- "/home/emiliano/Documents/ISP/proyectos/causality/causaLearner_R_pkg/causaLearner/pkg_causaLearner/experiments/QRM/learners/ANLSMN_margLikToo_v2/"
...
pm <- proc.time()
load(file=paste("./data/ANLSMN/", dataName, "_sims.RData", sep=""))
...
indxs <- (101:200)[-c(5,27,32,41,43,50,55,65,69,78)] 
dataList <- list(dags=dataList$dags[indxs], xs=dataList$xs[indxs], noiss=dataList$noiss[indxs], names=dataList$names[indxs])
ws <- ws[indxs]
ws <- ws/sum(ws)
...
dataTreatmentList <- c("stdr")#"gaussianiz"
...
indxNA <- which(sapply(scores, function(el) is.na(el$stdr$oracleME1$nnqr1_re_red[1])))
length(indxNA)
...
#version <- "vNC_CLASS"
#version <- "vNC_REG"
version <- "vMargLikToo_v2_nn" #"v_none" #"vNC_CORSPEARMAN"
...
rnkDecs <- deliverFuncToScore_recipeTree(scores=aggScores, func="rankedDecisions", ppTab=1, hypSc_nameProxy=names(scores[[1]][[1]][["oracleME1"]])[1])
...
n.boot <- 50
exps <- sapply(strsplit(dimnames(rnkDecsArr)$dataset, "\\."), function(el) el[1])
res <- sapply(unique(exps), function(exp){
  # exp <- "AN"
  print("***************")
  print(exp)
