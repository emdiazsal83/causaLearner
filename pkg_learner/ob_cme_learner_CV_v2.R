...
aux <- expand.grid(kernelX=c("kern_polyTnonlin"), # , "kern_quad", "kern_log"
                   kernelY=c("kern_van"), stringsAsFactors=F) #, "kern_quad", "kern_log"
...
kernelTab1 <- expand.grid(name=aux$name, centerLx=c(FALSE), centerKy=c(FALSE), optParms=c("none"), lambdaI=1:length(lambdas),  stringsAsFactors=F)
