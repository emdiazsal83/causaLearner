...
for(i in 1:nrow(kernelTab1)){
  ...
  lambdas_other <- list(val=NULL, seq=10^seq(-9,1, length.out=10))
  lambdasXY_other <- list(val=NULL, seq=10^seq(-9,1, length.out=10))
  ...
  optimizeParams_bin <- list(losses=list(negCE=negCE,  cmem_L2_f=cmem_L2_f), numFolds=5, testTrain="test")
  optimizeParams_other <- list(losses=list(negCE=negCE,  cmem_L2_f=cmem_L2_f), numFolds=5, testTrain="test")
  ...
  measureParamsFull <- list(PCEtr=list(func="PCEtr", aggFunc="sum", pars=list()),
  ...
  measureParamsRed  <- list(PCEtr=list(func="PCEtr", aggFunc="sum", pars=list()),
                            PCEte=list(func="PCEte", aggFunc="sum", pars=list()),
                            CCRtr=list(func="CCRtr", aggFunc="sum", pars=list()),
                            CCRte=list(func="CCRte", aggFunc="sum", pars=list()),
                            KCDC=list(func="KCDC", aggFunc="sum",pars=list()),
...
ppTab <- data.frame(id=c("KCDC","KCDCrel","KCDCpval","KCRDC","KCMC", "KCMCpval","KCSC", "KCSCpval","KCCC", "KCCCpval", "KCNSC",
                         "MCX","EDML","EDMB","TRE","PCE","PCEtr","PCEte","CCRtr","CCRte","CVTeRNE", "CVTrRNE", "EAL", "DAL"))
