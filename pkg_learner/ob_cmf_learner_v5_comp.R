...
print("enters ob_cmf_learner_v5.R")
...
aux <- expand.grid(kernelX=c("kern_rbf"), # ,"kern_rbfHkern_quad"
                   featureY=c("rff_rbf"), stringsAsFactors=F) #,"rff_rbfHrff_quad", "feat_polyTnonlin"
#aux <- aux[c(1,4),]
...
kernelTab1 <- expand.grid(name=aux$name, mainLoss=mainLosses, betaLearns=betaLearns, centerLx=c(FALSE), centerKy=c(FALSE), optParms=c("none", "lambda", "lambda_kernParsX", "lambda_kernParsXY"),  stringsAsFactors=F)
...
NCE_feats <- c("makeCME_cond_logRegfeatsK") # "makeLogRegFeats","makeCME_cond_logRegfeats", "makeCME_cond_logRegfeatsNorm", "makeCME_cond_logRegfeatsOdds", "makeCME_cond_feats"
...
NCE_classifiers <- c("logRegInt3") #"logRegInt1", "logRegInt2","logRegInt2","logRegInt2",, "cmeClass1"
...
kernelTab1$learnerName <- paste("cmfm", kernelTab1$name, kernelTab1$mainLoss, "DEL1", kernelTab1$feats, kernelTab1$classifier, "DEL2",
...
for(i in 1:nrow(kernelTab1)){
  ...
  parsYfeat <- lapply(aux, function(el){
    ...
    aux <- strsplit(el, "_")[[1]]
    if(aux[1]=="rff"){
      aux1 <- c("p_w","num_f", "seed", "map") #
      aux2 <- paste(aux1, aux[2], "Y", sep=".")
      names(aux2) <- aux1
      parsYfeat <- c(aux2)
    } else{
  ...
  lambdas_other <- list(val=NULL, seq=10^seq(-6,-1, length.out=2))
  lambdasXY_other <- list(val=NULL, seq=10^seq(-6,-1, length.out=2))
  ...
  # gammas <- list(val=NULL, seq=10^seq(-9,1, length.out=10))
  # lambdas_other <- list(val=NULL, seq=10^seq(-6,-1, length.out=5))
  # lambdasXY_other <- list(val=NULL, seq=10^seq(-6,-1, length.out=5))
  # lambdas_bin <- list(val=0, seq=0)
  # numX <- 5
  # numX_XY <- 5
  # numY_XY <- 10
  ...
  parsKernX <- lapply(kernelX_short, function(el) switch(el, bin=3, van=NA, rbf=NULL, quad=1e-3, log=0.98, poly=c(3,1,0) , laplace1=1e-3, laplace2=1e-3))
  parsFeatY <- lapply(featureY_short, function(el) switch(el, van=NA, rbf=NULL, quad=1e-3, log=0.98, polyTlin=c(1,1,0), polyTnonlin=c(3,1,0) , laplace1=1e-3))
  
  
  if(kernelTab1$optParms[i]=="none"){
    
    nms <- c("lambda", "kernelX", "featureY","centerLx", "centerKy", "gradX","NCE_learner", unlist(parsX), unlist(parsY), unlist(parsYfeat))
    nonOptimizableParams <- list(list(val=1e-4), list(val=kernelX[length(kernelX)]), list(val=featureY[length(featureY)]), list(val=centerLx), list(val=centerKy), list(val=gradX),list(val=NCE_learner))
    ...
    auxList <- lapply(1:length(dist_p_w), function(j) list(list(val=dist_p_w[j]), list(val=num_f[j]), list(val=1234), list(val=map[j])))
    auxList <- unlist(auxList, recursive=F)
    nonOptimizableParams <- c(nonOptimizableParams, auxList)
    ...
    nms <- c("kernelX", "featureY","centerLx", "centerKy", "gradX", "NCE_learner", unlist(parsX), unlist(parsY), unlist(parsYfeat))
    nonOptimizableParams <- list(list(val=kernelX[1]), list(val=featureY[1]),list(val=centerLx), list(val=centerKy), list(val=gradX), list(val=NCE_learner))
    for(j in 1:length(unlist(parsX))) nonOptimizableParams <- c(nonOptimizableParams, list(list(val=NULL, seq=NULL)))
    for(j in 1:length(unlist(parsY))) nonOptimizableParams <- c(nonOptimizableParams, list(list(val=NULL, seq=NULL)))
    auxList <- lapply(1:length(dist_p_w), function(j) list(list(val=dist_p_w[j]), list(val=num_f[j]), list(val=1234), list(val=map[j])))
    auxList <- unlist(auxList, recursive=F)
    ...
    nms <- c("featureY","centerLx", "centerKy", "gradX","NCE_learner", unlist(parsY), unlist(parsYfeat))
    nonOptimizableParams <- list(list(val=featureY[1]), list(val=centerLx), list(val=centerKy), list(val=gradX), list(val=NCE_learner))
    for(j in 1:length(unlist(parsY))) nonOptimizableParams <- c(nonOptimizableParams, list(list(val=NULL, seq=NULL)))
    auxList <- lapply(1:length(dist_p_w), function(j) list(list(val=dist_p_w[j]), list(val=num_f[j]), list(val=1234), list(val=map[j])))
    auxList <- unlist(auxList, recursive=F)
    nonOptimizableParams <- c(nonOptimizableParams, auxList)
    ...
    nms <- c("centerLx", "centerKy", "gradX","NCE_learner", unlist(parsYfeat))
    nonOptimizableParams <- list(list(val=centerLx), list(val=centerKy), list(val=gradX),list(val=NCE_learner))
    auxList <- lapply(1:length(dist_p_w), function(j) list(list(val=dist_p_w[j]), list(val=num_f[j]), list(val=1234), list(val=map[j])))
    auxList <- unlist(auxList, recursive=F)
    nonOptimizableParams <- c(nonOptimizableParams, auxList)
    nonOptimizableParams <- nonOptimizableParams[which(!is.na(nms))]
    ...
    optimizableParams <- list(lambdasXY, list(val=NULL, seq=kernelX), list(val=NULL, seq=featureY))
    if(kernelTab1$betaLearns[i] %in% c("learnBlambda_KCMC", "learnBlambda_KCSC")){
      nms <- c(nms, "gamma")
      optimizableParams <- c(optimizableParams, list(gammas))
    }
  ...
  #MisCR=MisCR, 
  losses <- list(cmem_L2_f=cmem_L2_f, cmem_L2_k=cmem_L2_k, KCDC=KCDC, KCMC=KCMC, KIIM=KIIM,KCRDC=KCRDC, KCSC=KCSC,KICSC=KICSC, KCNSC=KCNSC, KCCC_ent=KCCC_ent, KCCC_pca_ent=KCCC_pca_ent, KCCC_pca_ent2=KCCC_pca_ent2, KICCC_pca_ent=KICCC_pca_ent)
  ...
  if(! mainLossNm1 %in% names(losses)){
    lossesAux <- list(eval(parse(text=mainLossNm1)))
    names(lossesAux) <- mainLossNm1
    losses <- c(lossesAux, losses)
  }
  optimizeParams <- list(losses=losses, numFolds=5, testTrain="test", mainLoss=mainLossNm2)
  ...
  measureParamsFull <- list(gll_tr=list(func="gll_tr", aggFunc="sum", pars=list()),
                            ...
                            KCCC_pca_ent2=list(func="KCCC_pca_ent2", aggFunc="sum",pars=list()),
                            KICCC_pca_ent=list(func="KICCC_pca_ent", aggFunc="sum",pars=list()),
...
ppTab <- data.frame(id=c("KCDC","KCDCrel","KCDCpval","KCRDC","KCMC", "KIIM","KCMCpval","KCSC", "KICSC","KCSCpval","KCCC", "KICCC","KCCCpval", "KCNSC",
                         "MCX","EDML","EDMB","TRE","gll_tr","gll_te","L2_f_tr","L2_f_te","PCEtr","PCEte","CCRtr","CCRte","CVTeRNE", "CVTrRNE", "EAL", "DAL", "rankMajority","majority","HSIC"))
...
print("exits ob_cmf_learner_v5.R")
