...
print("enters ob_cme_learner_v5_ord.R")
...
# noise contratstative estimation - reg
mainLosses <- c("RMSE2", "gauss_log_lik", "cmem_L2_f", "cmem_L2_f_rel")

kernelTab1 <- expand.grid(name=aux$name, mainLoss=mainLosses, betaLearns=betaLearns, 
                          centerLx=c(FALSE), centerKy=c(TRUE), optParms=c("none", "lambda", "lambda_kernParsX", "lambda_kernParsXY"),  stringsAsFactors=F)
kernelTab1$feats <- "makeCME_cond_regfeatsK_x" #"makeLogRegFeats_x"
...
indx <- which(kernelTab1$mainLoss=="RMSE2")
NCE_feats <- c("makeCME_cond_regfeatsK") #"makeLogRegFeats",  "makeCME_cond_logRegfeats", "makeCME_cond_logRegfeatsNorm", "makeCME_cond_feats"
...
# spearman correlation to link cem to degree of fakeness
NCE_classifiers <- c("vanilla") #vanilla,krr1,"logRegInt1", "logRegInt2","logRegInt2", "cmeClass1"
...
kernelTab1$learnerName <- paste("cmem", kernelTab1$name, kernelTab1$mainLoss, "DEL1", kernelTab1$feats, kernelTab1$classifier, "DEL2",
...
for(i in 1:nrow(kernelTab1)){
  ...
  if(kernelTab1$optParms[i]=="none"){
    ...
    nms <- c("lambda", "kernelX", "kernelY","centerLx", "centerKy", "gradX","NCE_learner", unlist(parsX), unlist(parsY))
    nonOptimizableParams <- list(list(val=1e-4), list(val=kernelX[length(kernelX)]), list(val=kernelY[length(kernelY)]), list(val=centerLx), list(val=centerKy), list(val=gradX),list(val=NCE_learner))
    ...
    nms <- c("kernelX", "kernelY","centerLx", "centerKy", "gradX", "NCE_learner", unlist(parsX), unlist(parsY))
    nonOptimizableParams <- list(list(val=kernelX[1]), list(val=kernelY[1]),list(val=centerLx), list(val=centerKy), list(val=gradX), list(val=NCE_learner))
    for(j in 1:length(unlist(parsX))) nonOptimizableParams <- c(nonOptimizableParams, list(list(val=NULL, seq=NULL)))
    for(j in 1:length(unlist(parsY))) nonOptimizableParams <- c(nonOptimizableParams, list(list(val=NULL, seq=NULL)))
    nonOptimizableParams <- nonOptimizableParams[which(!is.na(nms))]
    names(nonOptimizableParams) <- na.omit(nms)
    ...
    nms <- c("kernelY","centerLx", "centerKy", "gradX","NCE_learner", unlist(parsY))
    nonOptimizableParams <- list(list(val=kernelY[1]), list(val=centerLx), list(val=centerKy), list(val=gradX), list(val=NCE_learner))
    for(j in 1:length(unlist(parsY))) nonOptimizableParams <- c(nonOptimizableParams, list(list(val=NULL, seq=NULL)))
    nonOptimizableParams <- nonOptimizableParams[which(!is.na(nms))]
    names(nonOptimizableParams) <- na.omit(nms)
    ...
    nms <- c("centerLx", "centerKy", "gradX","NCE_learner")
  ...
  losses <- list(cmem_L2_f=cmem_L2_f, cmem_L2_k=cmem_L2_k, KCDC=KCDC, KCMC=KCMC)
  mainLossNm1 <-  kernelTab1$mainLoss[i]
  mainLossNm2 <- mainLossNm1
  if(mainLossNm1 == "RMSE2"){
  ...
  measureParamsFull <- list(#gll_tr=list(func="gll_tr", aggFunc="sum", pars=list()),
                            ...
                            RMSEte=list(func="RMSEte", aggFunc="sum", pars=list()),
...
ppTab <- data.frame(id=c("KCDC","KCDCrel","KCDCpval","KCRDC","KCMC", "KCMCpval","KCSC", "KCSCpval","KCCC", "KCCCpval", "KCNSC",
                         "MCX","EDML","EDMB","TRE","gll_tr","gll_te","L2_f_tr","L2_f_te","RMSEte","PCEtr","PCEte","CCRtr","CCRte","CVTeRNE", "CVTrRNE", "EAL", "DAL", "rankMajority","majority"))
...
print("exits ob_cme_learner_v5.R")
