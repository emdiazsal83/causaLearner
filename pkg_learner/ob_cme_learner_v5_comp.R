...
print("enters ob_cme_learner_v5.R")
...
# noise contratstative estimation - class
...
# logistic regression to link cme to fake/real data
...
for(i in 1:nrow(kernelTab1)){
  ...
  if(kernelTab1$optParms[i]=="none"){
    ...
    nonOptimizableParams <- list(list(val=0.01), list(val=kernelX[length(kernelX)]), list(val=kernelY[length(kernelY)]), list(val=centerLx), list(val=centerKy), list(val=gradX),list(val=NCE_learner))
  ...
  losses <- list(cmem_L2_f=cmem_L2_f, cmem_L2_k=cmem_L2_k, KCDC=KCDC, KCMC=KCMC, KIIM=KIIM,KCRDC=KCRDC, KCSC=KCSC,KICSC=KICSC, KCNSC=KCNSC,KICCC_pca_ent2b=KICCC_pca_ent2b, KICCC_pca_entb=KICCC_pca_entb)
  ...
  if(kernelTab1$optParms[i]=="none") measureParams <- measureParamsRed2
...
ppTab <- data.frame(id=c("KCDC","KCDCrel","KCDCpval2","KCRDC","KCMC", "KIIM","KCMCpval","KCSC","KICSC", "KCSCpval","KCCC", "KICCC","KCCCpval", "KCNSC",
