...
#"rff_log"
aux <- expand.grid(kernelX=c("kern_van", "kern_rbf", "kern_quad", "kern_log", "kern_laplace2", "kern_laplace1","kern_poly"), 
                   featureY=c("feat_polyTlin","rff_rbf", "rff_quad","rff_laplace1", "feat_polyTnonlin"), stringsAsFactors=F)
aux$name <- with(aux, paste(kernelX, featureY , sep="AND"))

kernelTab1 <- expand.grid(name=aux$name, centerLx=c(FALSE, TRUE), centerKy=c(FALSE, TRUE), optParms=c("none", "lambda", "lambda_kernParsX", "lambda_kernParsXY"),  stringsAsFactors=F)
...
ppTab$rank2Funcs <- c("quot","addQuot","differ")[c(2,2,2,2,2,2,2,2,2,2,2)]
ppTab$rankFuncs <- rep("correctScoreToAdd", 11)
ppTab$probFuncs <- rep("scoreToProb", 11)
ppTab$argTypes <- c(rep("cmfs", 11))
