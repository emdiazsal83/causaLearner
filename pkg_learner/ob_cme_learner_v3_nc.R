...
aux <- expand.grid(kernelX=c("kern_rbf"), 
                   kernelY=c("kern_rbf"), stringsAsFactors=F)


aux$name <- with(aux, paste(kernelX, kernelY , sep="AND"))

kernelTab1 <- expand.grid(name=aux$name, centerLx=c(FALSE), centerKy=c(TRUE), optParms=c("none", "lambda", "lambda_kernParsX", "lambda_kernParsXY"), stringsAsFactors=F)
...
for(i in 1:nrow(kernelTab1)){
  ...
  if(kernelTab1$optParms[i]=="none"){
    ...
    nonOptimizableParams <- list(list(val=0.01), list(val=centerLx), list(val=centerKy))
  ...
  cmem_learner_aux <- constructLearner(hyperParams, learnParams, optimizeParams, heuristicSet, optimizeSet, learn.cmem, predict.cmem, msrs=measureParams, calcMsrs=calcMsrs,  makeKernel=makeKernel)
