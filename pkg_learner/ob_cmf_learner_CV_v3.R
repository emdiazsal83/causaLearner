...
aux <- expand.grid(kernelX=c("kern_van", "kern_rbf", "kern_quad", "kern_laplace1","kern_polyTnonlin"), 
                   featureY=c("kern_van", "kern_rbf", "kern_quad", "kern_laplace1","kern_polyTnonlin"), stringsAsFactors=F)
...
indxY <- match(kernelTab1$featureY_short_short, paramTab2$kernel)
...
for(i in 1:nrow(kernelTab1)){
  ...
  parsfeatY <- switch(kernelTab1$featureY_short[i], van=vanY, rbf=rbfY, quad=quadY, log=logY, polyTlin=polyTlinY, polyTnonlin=polyTnonlinY , laplace1=laplace1Y)
  
  # formatC(as.numeric(sapply(0:26, dec2base, base=3)), width=3, flag="0")  
  indxX <- as.numeric(strsplit(dec2base(kernelTab1$kernParsX_indx[i]-1, base=numKernParsX),"")[[1]])+1
  parsKernX <- mapply(function(el, indx) el[indx], el=parsKernX, indx=indxX)
  indxY <- as.numeric(strsplit(dec2base(kernelTab1$kernParsY_indx[i]-1, base=numKernParsY),"")[[1]])+1
  ...
  nonDataParams <- list(kernelX=list(name=kernelTab1$kernelX[i], pars=na.omit(parsX)), 
                        featureY=list(name=kernelTab1$featureY[i], pars=na.omit(parsY)))
