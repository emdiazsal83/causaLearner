...
paramTab1 <- data.frame(kernel=c("rbf", "quad", "log"), parameter=c("sigma", "offset", "degree"), 
                       paramFunc=c("getFixedParams_rbf2", "getFixedParams_quad", "getFixedParams_log"), stringsAsFactors=F)

paramTab2 <- data.frame(kernel=c("rbf", "quad", "log"), parameter=c("sigma", "offset", "degree"), 
                        paramFunc=c("getFixedParams_rbf2", "getFixedParams_quad", "getFixedParams_log"), stringsAsFactors=F)

aux <- expand.grid(kernelX=c("kern_rbf", "kern_quad", "kern_log"), kernelY=c("kern_rbf", "kern_quad", "kern_log"), stringsAsFactors=F)
aux$name <- with(aux, paste(kernelX, kernelY , sep="AND"))

kernelTab1 <- expand.grid(name=aux$name, centerLx=c(FALSE), centerKy=c(FALSE), optParms=c("none", "lambda", "lambda_kernParsX", "lambda_kernParsXY"), stringsAsFactors=F)
...
for(i in 1:nrow(kernelTab1)){
  ...
  if(kernelTab1$optParms[i]=="none"){
    ...
    parsKernX <- switch(kernelX_short, rbf=1e4, quad=1e-3, log=0.98)
    parsKernY <- switch(kernelY_short, rbf=1e4, quad=1e-3, log=0.98)
