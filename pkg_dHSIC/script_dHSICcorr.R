# show correction to dHSIC bug when passing ready-made kernel matrices using the "K" parameter
remove(list=ls())
...
dhsic2 <- function (X, Y, K, kernel = "gaussian", bandwidth = 1, matrix.input = FALSE) {
  if (missing(K)) {
    if (!missing(Y)) {
      X <- list(X, Y)
    }
...
dhsic.test2 <- function (X, Y, K, alpha = 0.05, method = "permutation", kernel = "gaussian", 
...
n <- 100
x <- rnorm(n)
y <- 0.5*x^2 + rnorm(n, sd=0.1)
plot(x, y)

sd1 <- 1
kernelX <- do.call("rbfdot", list(sigma=sd1))
kernelY <- do.call("rbfdot", list(sigma=sd1))
